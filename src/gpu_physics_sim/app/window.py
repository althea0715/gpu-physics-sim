...
class Window:
    def __init__(self, title: str, width: int, height: int):
        ...
        self._fn_on_key: Callable[[int, bool], None] | None = None
    ...
    def _key_callback(self, _: GLFWwindow, key: int, code: int, action: int, mods: int):
        if self._fn_on_key:
            self._fn_on_key(key, action == GLFW_PRESS)

    def should_close(self) -> bool:
        return glfwWindowShouldClose(self.window)

    def request_close(self):
        logger.info("request_close")
    ...
    def set_key_callback(self, fn: Callable[[int, bool], None]):
        self._fn_on_key = fn
